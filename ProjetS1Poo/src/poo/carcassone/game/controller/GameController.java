...
public class GameController extends JFrame implements MouseMotionListener, MouseListener, Serializable {
    ...
    public void mouseDragged(MouseEvent mouseEvent) {
         ...
         * System.out.println(gameView.getjPartisan().getX() + " " +
         * gameView.getjPartisan().getY());
         * }
         */
    }
    ...
    private Point getCoordinate(MouseEvent event) {
        ...
        int tuileWidth = gameView.getJTuile().getWidth() + 5, tuileHeight = gameView.getJTuile().getHeight() + 5; // 5
                                                                                                                  // c'est
                                                                                                                  // l'espace
                                                                                                                  // entre
                                                                                                                  // les
    ...
    private void resetTuilePosition() {
        gameView.getJTuile().setLocation((int) (0.5 * getWidth() + getInsets().left),
                (int) (0.8 * getHeight() + getInsets().top));
    }

    private void resetPartisanPosition() {
        gameView.getjPartisan().setLocation((int) (0.7 * getWidth() + getInsets().left),
                (int) (0.8 * getHeight() + getInsets().top));
    }
    ...
    private Joueur getWinner() {
        ...
        for (Joueur joueur : joueurs) {
            if (joueur.getNbPoints() > res.getNbPoints())
                res = joueur;
        }
        return res;
    }
