...
public class Tuile implements Serializable {
    ...
    private void abbayeOuCarrefour() {
        ...
        for (Paysage p : cotes) {
            if (p.getClass() == Pres.class)
                nbPres++;
            else if (p.getClass() == Chemin.class)
                nbChemins++;
        }
        if (nbPres > 3)
            abbaye = true;
    ...
    /**
     * @param cote     le côté où on va poser le partisan 0 est gauche 1 est haut 2
     *                 est droite 3 est bas et 4 est au milieu
    ...
    public String toString() {
        return cotes[0].toString() + "_" + cotes[1].toString() + "_" + cotes[2].toString() + "_" + cotes[3].toString()
                + (bouclier ? "_b" : "");
    }

    public boolean corresponds(Plateau p, int i, int j) {
        if (p.getTuile(i, j) != null)
